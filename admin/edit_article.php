...
if (isset($_GET['id']) && !empty($_GET['id'])) {
  ...
  $fetchSql = "SELECT * FROM articles WHERE id = ?";
  ...
  $errorMsg = "No article ID provided";
}
...
if (isset($_POST['update_article'])) {
  ...
  if (empty($title)) {
    ...
    $errorMsg = "Article excerpt cannot be empty";
  } else {
    // Check if new slug already exists (only if title changed)
    if ($slug_changed) {
      $checkSlugSql = "SELECT id FROM articles WHERE slug = ? AND id != ?";
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
      ...
      if (in_array(strtolower($filetype), $allowed)) {
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $upload_path)) {
          ...
          $image = 'uploads/articles/' . $new_filename;
        } else {
    ...
    if (empty($errorMsg)) {
      // Update article
      $updateSql = "UPDATE articles SET title = ?, slug = ?, author = ?, author_role = ?, 
  ...
  <div class="main-content" id="mainContent">
    <div class="row mb-4">
      ...
      <div class="col-md-6 text-md-end">
        <a href="manage-articles.php" class="btn btn-outline-secondary">
      ...
      <div class="card border-0 shadow-sm">
        <div class="card-body p-4">
          <form method="post" action="" enctype="multipart/form-data">
            ...
            <input type="hidden" name="current_slug" value="<?php echo htmlspecialchars($article['slug']); ?>">

            <div class="row">
              <!-- Left Column - Basic Info -->
              <div class="col-md-8">
                <div class="mb-4">
                  <label for="title" class="form-label required-field">Article Title</label>
                  <input type="text" class="form-control" id="title" name="title" value="<?php echo htmlspecialchars($title); ?>" required>
                </div>

                <div class="mb-4">
                  <label for="content" class="form-label required-field">Article Content</label>
                  <textarea class="form-control" id="content" name="content" rows="12" required><?php echo htmlspecialchars($content); ?></textarea>
                </div>

                <div class="mb-4">
                  <label for="excerpt" class="form-label required-field">Article Excerpt</label>
                  <p class="text-muted small">A short summary that appears in article listings (max 200 characters)</p>
              ...
              <div class="col-md-4">
                <div class="card mb-4">
                  ...
                  <div class="card-body">
                    ...
                    <div class="mb-3">
                      ...
                      <input type="text" class="form-control" id="author_role" name="author_role" value="<?php echo htmlspecialchars($author_role); ?>" placeholder="e.g. Tech Writer, Editor" required>
                    </div>
                    ...
                    <div class="mb-3">
                      ...
                      <div class="form-text">e.g. technology, science, news</div>
                    </div>
                    ...
                    <div class="mb-3">
                      ...
                      <div class="text-muted small">Featured articles appear in highlighted sections</div>
                    </div>
                  </div>
                </div>
                ...
                <div class="d-grid gap-2">
                  <button type="submit" name="update_article" class="btn btn-primary btn-lg">Update Article</button>
                  <a href="manage-articles.php" class="btn btn-outline-secondary">Cancel</a>
                </div>
              </div>
            </div>
          </form>
