...
if (isset($_GET['id']) && !empty($_GET['id'])) {
  ...
  // Fetch story details
  $fetchSql = "SELECT * FROM stories WHERE id = ?";
...
if (isset($_POST['update_story'])) {
  ...
  if (empty($title)) {
    ...
    if (empty($errorMsg)) {
      // Update story
      $updateSql = "UPDATE stories SET title = ?, author = ?, author_role = ?, 
                    content = ?, excerpt = ?, image = ?, featured = ? WHERE id = ?";
      ...
      if ($updateStmt->execute()) {
        $successMsg = "Story updated successfully";
...
<head>
  ...
  <title>Edit Story - Admin Panel</title>
  <!-- Bootstrap 5 CSS -->
            ...
            <div class="row">
              ...
              <div class="col-md-4">
                ...
                <div class="d-grid gap-2">
                  <button type="submit" name="update_story" class="btn btn-primary btn-lg">Update Story</button>
                  <a href="manage-stories.php" class="btn btn-outline-secondary">Cancel</a>
                </div>
              </div>
            </div>
          </form>
